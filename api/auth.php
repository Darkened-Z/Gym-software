        ...
        case 'login':
            if ($method === 'POST') {
                ...
                if (!empty($username) && !empty($password)) {
                    ...
                    if ($result) {
                        // Staff (not admin) may be limited to their own allowed days/hours.
                        if (($result['role'] ?? '') === 'staff') {
                            $acc = AuthHelper::staffAccessState($db, (int)$result['id']);
                            if ($acc['restricted'] && !$acc['open']) {
                                http_response_code(403);
                                echo json_encode([
                                    'success' => false,
                        ...
                        $_SESSION['staff_level'] = (int)($result['staff_level'] ?? 1);
        ...
        case 'check':
            if (isset($_SESSION['role'])) {
                ...
                if (in_array($_SESSION['role'], ['admin', 'staff'], true)) {
                    ...
                    $response['staff_level'] = ($_SESSION['role'] === 'admin') ? 1 : (int)($_SESSION['staff_level'] ?? 1);
